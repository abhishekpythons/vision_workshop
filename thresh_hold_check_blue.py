...
def track_ball(frame):
    global threshold_x, threshold_y, threshold_width, threshold_height, prev_ball_x, prev_ball_y, commanding

    # Convert BGR to HSV
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

    # Define range of blue color in HSV
    lower_red = np.array([41, 132, 132])
    ...
    if len(contours) > 0:
        ...
        if not is_inside:
            ...
            # print("Direction:", direction)
                requests.get(base_url+direction)
                commanding=True
